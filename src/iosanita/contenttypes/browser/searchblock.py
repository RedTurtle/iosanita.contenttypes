...
class SearchBlockDownload(BrowserView):
    ...
    def export_csv(self, results, columns):
        ...
        self.request.response.setHeader(
            "Content-Disposition", f"attachment; filename=export_{self.block_id}.csv"
        )
    ...
    def export_pdf(self, results, columns):
        ...
        pdf_file = HTML(string=html_content, base_url=self.request.URL).write_pdf(
        ...
        self.request.response.setHeader(
            "Content-Disposition", f"attachment; filename=export_{self.block_id}.pdf"
        )
        return pdf_file
